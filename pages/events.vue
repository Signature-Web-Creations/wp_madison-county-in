...
export default {
  ...
  computed: {
    ...
    selected() {
      ...
      return this.eventList.find((organization) => organization.id === id)
    },
  },
  ...
  methods: {
    goTo() {
      const id = this.active[0]
      const organization = this.eventList.find(
    ...
    async fetchOrganizations(item) {
      ...
      if (this.tab == 1) {
        return this.eventList.filter((organization) => {
    ...
    getCategoryList() {
      ...
      this.eventList.forEach((organization) => {
        organization.categories.forEach((category) => {
